...
Default local Docker and Podman deployments put the browser and the server on separate loopback networks, so the OAuth callback never arrived. There is now a temporary `compose.codex-oauth.yaml` overlay (and the equivalent `docker`/`podman` commands) that publishes ports `1455`/`1457` only while you sign in — with teardown, so those ports go back to your other Codex clients. See [CONTAINERIZATION.md](https://github.com/HKUDS/DeepTutor/blob/main/docs-for-user/CONTAINERIZATION.md#temporary-local-codex-oauth-bridge).
