...
Профили LLM, совместимые с OpenAI, также предлагают настройку **протокола API**. Оставьте `Auto` для обычного определения провайдера и совместимого резервного варианта, выберите `Responses API` для конечных точек, реализующих только `/responses`, или `Chat Completions` для конечных точек, которым требуется `/chat/completions`. Принудительный режим Responses работает с отказом по умолчанию: ошибки конечной точки возвращаются напрямую, а не приводят к незаметной повторной попытке через Chat Completions. Соответствующее поле профиля в `model_catalog.json` — `wire_api` (`auto`, `responses` или `chat_completions`).
