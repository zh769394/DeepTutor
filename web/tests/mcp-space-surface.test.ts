...
test("toggling one server writes only that server", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      ["PUT /api/space/mcp/servers/tavily?dt_workspace="],
    );
...
test("a rename adds the new server before removing the old one", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      stub.calls.map((call) => `${call.method} ${call.url}`),
      ["PUT /api/space/mcp/servers/exa2?dt_workspace=", "DELETE /api/space/mcp/servers/exa?dt_workspace="],
    );
  } finally {
    stub.restore();
  }
...
test("a refused rename never reaches the delete", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      stub.calls.map((call) => `${call.method} ${call.url}`),
      ["PUT /api/space/mcp/servers/exa2?dt_workspace="],
    );
  } finally {
    stub.restore();
  }
...
test("a name with URL-significant characters is escaped in the path", async () => {
  ...
  try {
    await writeMcpSurface(SPACE_MCP_SURFACE, previous, next);
    assert.equal(stub.calls[0].url, "/api/space/mcp/servers/a%20b?dt_workspace=");
  } finally {
    stub.restore();
  }
});
...
test("removing the last server still refreshes from the response", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      stub.calls.map((call) => `${call.method} ${call.url}`),
      ["DELETE /api/space/mcp/servers/exa?dt_workspace="],
    );
...
test("a no-op write re-reads instead of PUTting anything", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      stub.calls.map((call) => `${call.method} ${call.url}`),
      ["GET /api/space/mcp/servers?dt_workspace="],
    );
  } finally {
    stub.restore();
  }
...
test("the admin registry keeps its whole-map PUT", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      stub.calls.map((call) => `${call.method} ${call.url}`),
      ["PUT /api/settings/mcp?dt_workspace="],
    );
