...
function installWindow(): { redirectedTo: () => string | null } {
  ...
  const location = { pathname: "/knowledge-bases", href: "", search: "", origin: "http://localhost" };
...
test("listImaKnowledgeBases sends credentials and preserves pagination", async () => {
  ...
  try {
    ...
    assert.equal(captured.input, "/api/knowledge-bases/list-ima?dt_workspace=");
...
test("connectImaKnowledgeBase sends the DeepTutor name and invalidates on success", async () => {
  ...
  try {
    ...
    assert.equal(captured.input, "/api/knowledge-bases/connect-ima?dt_workspace=");
